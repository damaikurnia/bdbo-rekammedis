...
public class ControlData {
    ...
    public static String NamaPasien(final String id) throws SQLException {
        String cek = "";
        List<Pasien> persons = db.query(new Predicate<Pasien>() {

            @Override
            public boolean match(Pasien person) {
                return person.getId().equalsIgnoreCase(id);
            }
        });

        // Menampilkan hasil query
        for (Pasien person : persons) {
            cek = (person.getNama());
        }
...
//            pas.setFakultas((String) res.get(3));
    ...
    public static List<RekamMedisData> getAllRMPasien(String id) throws SQLException {
        ...
        ObjectSet res = query.execute();
        while (res.hasNext()) {
            pas.add((RekamMedisData) res.next());
        }
        return pas;
    }
